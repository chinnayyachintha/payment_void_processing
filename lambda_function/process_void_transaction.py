...
def validate_transaction(transaction):
    """Validate if the transaction is eligible for voiding."""
...
def create_void_transaction(original_transaction, void_amount, reason):
    ...
    # Create the void transaction, excluding unwanted fields
    void_transaction = {
        ...
        # Add new attributes here if needed
    }
    
    try:
        # Insert the void transaction into DynamoDB
...
def log_audit_trail(original_transaction, void_transaction, user_id, reason):
    ...
    try:
        # Log the audit entry into the audit trail table
